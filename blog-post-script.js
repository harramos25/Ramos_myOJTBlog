    ...
    if (slides.length > 0) {
        ...
        indicators.forEach((indicator, index) => {
            indicator.addEventListener('click', () => {
                currentSlide = index;
                showSlide(currentSlide);
                // remove resetAutoPlay call
            });
        });

        // Auto-play (DISABLED)
        function startAutoPlay() {
             // autoPlayInterval = setInterval(nextSlide, 4000); // Disabled
        }
        ...
        if (slides.length > 1) {
            ...
            if (carouselWrapper) {
                // Remove listeners or do nothing as auto-play is disabled
